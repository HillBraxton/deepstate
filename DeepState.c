...
void makeFilename(char *name, size_t size) {
  uint32_t seeds[5] = {
    2166136261u,
    2166136261u ^ 0x9e3779b9u,
    2166136261u ^ 0x85ebca6bu,
    2166136261u ^ 0xc2b2ae35u,
    2166136261u ^ 0x27d4eb2fu,
  };

  size_t offset = 0;

  for (int j = 0; j < 5 && offset + 8 <= size; j++) {
    uint32_t hash = seeds[j];

    for (size_t i = 0; i < DeepState_InputIndex; i++) {
      hash ^= (unsigned char) DeepState_Input[i];
      hash *= 16777619u;
    }

    snprintf(name + offset, size - offset + 1, "%08x", hash);
    offset += 8;
  }

  if (offset < size) {
    memset(name + offset, '0', size - offset);
  }
}
